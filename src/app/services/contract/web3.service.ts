...
export class Web3Service {
  ...
  async connectToContract(){
    ...
    this.nftContract = new this.web3js.eth.Contract(BretflixNFTContract.abi, '0xe3Db97afc41721da4E9dF63ab2f9C9cf1DbE615a');
    this.isInitialized = true;
  }

  async mintToken(account: any[]){
    if(!this.isInitialized){
      await this.connectToContract();
    }
    
    return this.nftContract.methods.mint(account[0], account[0]).send({from: account[0]});
  }
